...
import numpy as np
from utils_dudson import file_numbers, writef
...
def write(f, data):
    ...
    # Write description
    description = f'HEQ gfile tttttttttttttttttttttttttttttttttttttt{0:4}{nx:4}{ny:4}'
    f.write(description + '\n')
    ...
    writef(f, data['FFprime'])
    writef(f, data['Pprime'])
    writef(f, data['psi'])
    writef(f, data['qpsi'])
    f.write("{:5d}".format(data['np_lcfs']))
    f.write("{:5d}".format(data['nlim']))
    f.write('\n')
    writef(f, data['lcfs'])
    writef(f, data['limiter'])
